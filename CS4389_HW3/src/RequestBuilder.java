...
public class RequestBuilder {
	 ...
	 */
	public static Set<Subject> setupSubjects(String userName, String groupName)
			throws URISyntaxException {
		...
		StringAttribute value = new StringAttribute(userName);
		...
		attributes
				.add(new Attribute(new URI("group"), "admin@users.example.com",
						null, new StringAttribute(groupName)));
	 ...
	 *             if there is a problem with a URI
	 */
	public static Set<Attribute> setupResource(String resourceName)
			throws URISyntaxException {
		HashSet<Attribute> resource = new HashSet<Attribute>();

		// the resource being requested
		AnyURIAttribute value = new AnyURIAttribute(new URI(resourceName));
	...
	public void run(String XMLFile, String user, String file,
			ArrayList<String> outFileNames) throws FileNotFoundException,
			URISyntaxException {
		ExtractGroupNames gp = new ExtractGroupNames(XMLFile, user);
		gp.run();
		ArrayList<String> groups = gp.getGroups();
		...
		for (int i = 0; i < groups.size(); i++) {
			...
			RequestCtx request = new RequestCtx(setupSubjects(user,
					(String) groups.get(i)), setupResource(file),
					setupAction(), new HashSet<Object>());
	...
	/**
	 * Command-line interface that creates a new Request by invoking the static
	 * methods in this class. The Request has no Environment section.
	 */
